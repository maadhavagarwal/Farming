...
import coursesContext from '../context/coursesContext';
 ...
 let navigate = useNavigate();
  ...
  const {  getAdminCourses,adminCourses,addCourses,deleteCourses,getCoursesEnrollmentsAdmin } = context;
   ...
   return(
    ...
    <Row>
      ...
      <Form onSubmit={handleClick}>
        ...
        {
            Messages&&(
              <Messages variant="success">{message}</Messages>)
        }
...
{Array.isArray(adminCourses) && adminCourses.map((course) => (
                <tr key={course._id}>
                  ...
                  <td>{course._id}</td>
                  <td>{course.name}</td>
                  <td><img src={course.image} alt="" width="100px" height="100" /></td>
                
                  <td>{course.price}</td>
                 
                  <td><Button variant="danger success" onClick={() => handleDelete(course._id)}>Delete</Button></td>
                </tr>
              ))}
